...
export default function FinanzasPage() {
  ...
  const { data: transactions = [], isLoading } = useQuery<FinancialTransaction[]>({
    ...
    queryFn: async () => (await api.get<FinancialTransaction[]>("/finance", { params: { limit: 1000 } })).data,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: TransactionFormData) => {
      ...
      return editingTransaction
        ? api.patch(`/finance/${editingTransaction.id}`, payload)
        : api.post("/finance", payload);
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => api.delete(`/finance/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["transactions"] });
  ...
  const openModal = (t?: FinancialTransaction) => {
    if (t) {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Control de ingresos, egresos y balances de operación.
          </p>
        </div>
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div
                        className={`h-10 w-10 rounded-xl flex items-center justify-center border border-white/5 ${t.type === "INCOME" ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"}`}
                      >
                        {t.type === "INCOME" ? (
