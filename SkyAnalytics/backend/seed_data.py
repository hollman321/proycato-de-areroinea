...
def insertar_con_copy_from(
    conexion: psycopg2.extensions.connection, df: pd.DataFrame, batch_num: int
):
    """
    ...
    try:
        ...
        # Usar COPY FROM
        with conexion.cursor() as cur:
            cur.copy_from(buffer, "pasajeros", columns=df.columns, sep="|")

        conexion.commit()
        logger.info(
            f"  Batch {batch_num}: {len(df):,} registros insertados con COPY FROM"
        )
...
def cargar_datos(total_registros: int, truncate: bool = False):
    ...
    try:
        while registros_insertados < total_registros:
            ...
            # Generar lote
            df = generar_lote_pasajeros(
                cantidad_batch, inicio_global=registros_insertados
            )
            ...
            velocidad = (
                registros_insertados / tiempo_transcurrido
        ...
        velocidad_promedio = (
            registros_insertados / tiempo_total if tiempo_total > 0 else 0
        )
        ...
        if velocidad_promedio > 0:
            logger.info(
                f"  Tiempo estimado para 10M: {10_000_000 / velocidad_promedio / 60:.1f} minutos"
            )
        else:
            logger.info(
                "  Tiempo estimado para 10M: no disponible (sin registros insertados)"
            )
        logger.info(f"{'='*60}\n")
...
def main():
    ...
    parser.add_argument(
        "--test", action="store_true", help="Modo test: solo genera 1,000 registros"
    )
    parser.add_argument(
        "--truncate", action="store_true", help="Trunca tabla antes de insertar"
    )
