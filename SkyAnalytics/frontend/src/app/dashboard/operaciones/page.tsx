...
const STATUS_OPTIONS = [
    ...
    { value: 'IN_PROGRESS', label: 'En Proceso', color: 'bg-sky-500/10 text-sky-400 border-sky-500/20' },
    { value: 'COMPLETED', label: 'Completado', color: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' },
    { value: 'CANCELLED', label: 'Cancelado', color: 'bg-rose-500/10 text-rose-400 border-rose-500/20' },
]
        ...
        <div className="space-y-8">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p className="text-slate-400 mt-1">Monitoreo y gestión de flujos logísticos en tiempo real.</p>
                </div>
            ...
            <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                <StatCard
                    ...
                    value={operations?.filter((o: any) => o.status === 'IN_PROGRESS').length || 0}
                ...
                <StatCard
                    title="Pendientes"
                    value={operations?.filter((o: any) => o.status === 'PENDING').length || 0}
                ...
                <StatCard
                    ...
                    value={operations?.filter((o: any) => o.status === 'COMPLETED').length || 0}
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {operations?.map((op: any) => {
                                const statusStyle = STATUS_OPTIONS.find(s => s.value === op.status)
                                return (
                                    <tr key={op.id} className="hover:bg-white/[0.02] transition-colors group">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <button onClick={() => deleteMutation.mutate(op.id)} className="p-2 hover:bg-rose-500/10 rounded-lg text-slate-400 hover:text-rose-400 transition-all"><Trash2 className="h-4 w-4" /></button>
                                            </div>
                                        </td>
                                    </tr>
                                )
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-md rounded-3xl border border-white/10 bg-slate-900 p-8 shadow-2xl">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-xl font-bold text-white">{editingOp ? 'Actualizar Operación' : 'Nueva Operación'}</h2>
                            ...
                            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(formData); }}>
                                <div>
                                    <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Título de Operación</label>
                                    <input type="text" required className="mt-1 w-full rounded-xl bg-slate-950 border border-white/10 p-3 text-sm text-white focus:ring-2 focus:ring-sky-500/50" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} />
                                </div>
                                ...
                                <div>
                                    <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Estado Actual</label>
                                    <select className="mt-1 w-full rounded-xl bg-slate-950 border border-white/10 p-3 text-sm text-white focus:ring-2 focus:ring-sky-500/50" value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value })}>
                                ...
                                <div>
                                    ...
                                    <textarea rows={3} className="mt-1 w-full rounded-xl bg-slate-950 border border-white/10 p-3 text-sm text-white focus:ring-2 focus:ring-sky-500/50" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} />
                                </div>

                                <div className="pt-4 flex gap-3">
                                    <Button type="button" variant="ghost" className="flex-1" onClick={closeModal}>Cancelar</Button>
                                    <Button type="submit" className="flex-1" loading={saveMutation.isPending}>{editingOp ? 'Actualizar' : 'Registrar'}</Button>
                                </div>
                            </form>
                        </motion.div>
                    </div>
